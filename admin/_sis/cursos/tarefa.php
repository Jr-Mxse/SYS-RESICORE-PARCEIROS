...
$ClassName = filter_input(INPUT_GET, 'task', FILTER_SANITIZE_STRING);
if (!$ClassName):
    $_SESSION['wc_ead_alert'] = ["yellow", "Oppps {$user_name}, erro ao acessar:", "Não foi possível identificar a aula que você tentou acessar :/"];
    header("Location: dashboard.php?wc=cursos/curso&slug={$course_name}");
    exit;
else:
    $Read->ExeRead(DB_EAD_CLASSES, "WHERE class_name = :name AND course_id = :course", "name={$ClassName}&course={$course_id}");
    if (!$Read->getResult()):
        $_SESSION['wc_ead_alert'] = ["yellow", "Oppps {$user_name}, erro ao acessar:", "Não foi possível identificar a aula que você tentou acessar :/"];
        header("Location: dashboard.php?wc=cursos/curso&slug={$course_name}");
        exit;
...
if (!$Read->getResult()):
    ...
    if (!empty($enrollment_bonus)):
        ...
        if (!empty($Read->getResult()) && !empty($Read->getResult()[0]['enrollment_end']) && time() >= strtotime($Read->getResult()[0]['enrollment_end'])):
            $_SESSION['wc_ead_alert'] = ['red', "Oppsss, bônus bloqueado:", "Desculpe {$user_name}, mas o curso que liberou o bônus expirou."];
            header("Location: dashboard.php?wc=cursos/home");
            exit;
        endif;
    elseif (!empty($enrollment_end) && time() >= strtotime($enrollment_end)):
        $_SESSION['wc_ead_alert'] = ['red', "Oppsss, acesso expirado:", "Desculpe {$user_name}, mas sua assinatura do curso {$course_title} expirou dia " . date("d/m/y \a\s H\hi", strtotime($enrollment_end)) . "."];
        header("Location: dashboard.php?wc=cursos/home");
        exit;
    endif;
...
if (!$Read->getResult()):
    $_SESSION['wc_ead_alert'] = ['yellow', "Erro ao acessar:", "Não foi possível identificar o módulo da aula {$class_title}."];
    header("Location: dashboard.php?wc=cursos/curso&slug={$course_name}");
    exit;
else:
    extract($Read->getResult()[0]);

    // Módulo obrigatório pendente
    $Read->FullRead("
        SELECT COUNT(c.class_id) as class_pending, m.module_title
        ...
        WHERE m.course_id = :cs
          ...
          AND c.class_id NOT IN(
              SELECT class_id FROM " . DB_EAD_STUDENT_CLASSES . "
              WHERE course_id = :cs AND user_id = :us AND student_class_check IS NOT NULL
          )
        GROUP BY m.module_title
        ORDER BY m.module_order ASC, c.class_order ASC
        LIMIT 1
    ", "cs={$course_id}&mo={$module_order}&us={$user_id}");

    $ClassesPendent = ($Read->getResult() ? $Read->getResult()[0]['class_pending'] : 0);
    if ($ClassesPendent > 0 && $_SESSION['userLogin']['user_level'] <= 5):
        $_SESSION['wc_ead_alert'] = ['red', "Módulo {$Read->getResult()[0]['module_title']} obrigatório!", "Existem aulas pendentes, {$user_name}. Marque todas como concluídas antes de continuar!"];
        header("Location: dashboard.php?wc=cursos/curso&slug={$course_name}");
        exit;
    endif;

    // Bloqueio por data
    $ReleaseUnlock = !empty($module_release_date)
        ? strtotime($module_release_date)
        : strtotime($enrollment_start . "+{$module_release}days");

    $ModuleUnlocked = (($ReleaseUnlock <= time() && $ClassesPendent == 0) ? true : false);
    if (!$ModuleUnlocked && $_SESSION['userLogin']['user_level'] < 6):
        $_SESSION['wc_ead_alert'] = ['red', "Módulo {$module_title} bloqueado:", "{$user_name}, acesse este módulo a partir de " . date("d/m/y \a\s H\hi", $ReleaseUnlock) . "."];
        header("Location: dashboard.php?wc=cursos/curso&slug={$course_name}");
        exit;
    endif;
endif;
...
if ($Read->getResult()):
    ...
    $CreateStudentClass = [
        ...
        'course_id' => $course_id,
        'class_id' => $class_id,
        'student_class_play' => $DateThis,
        'student_class_views' => 1
    ];

    if (!$class_video):
        $CreateStudentClass['student_class_free'] = 1;
        $CreateStudentClass['student_class_check'] = $DateThis;
    endif;
...
$ClassPreview = "<a class='icon-arrow-left class_next' href='dashboard.php?wc=cursos/curso&slug={$course_name}#play' title='Voltar ao Índice do Curso'>Voltar ao Índice do Curso</a>";
...
if ($Read->getResult()):
    $ClassPreview = "<a class='icon-arrow-left class_next' href='dashboard.php?wc=cursos/tarefa&task={$Read->getResult()[0]['class_name']}#play' title='{$Read->getResult()[0]['class_title']}'>" . Check::Chars($Read->getResult()[0]['class_title'], 32) . "</a>";
endif;

$ClassNext = "<a class='icon-arrow-right class_next' href='dashboard.php?wc=cursos/curso&slug={$course_name}#play' title='Voltar ao Índice do Curso'>Voltar ao Índice do Curso</a>";
...
if ($Read->getResult()):
    $ClassNext = "<a class='icon-arrow-right class_next' href='dashboard.php?wc=cursos/tarefa&task={$Read->getResult()[0]['class_name']}#play' title='{$Read->getResult()[0]['class_title']}'>" . Check::Chars($Read->getResult()[0]['class_title'], 32) . "</a>";
endif;
...
<header class="dashboard_header">
    <div class="dashboard_header_title">
        ...
        <p class="dashboard_header_breadcrumbs">
            <a href="dashboard.php?wc=cursos/curso&slug=<?= $course_name; ?>" title="<?= $course_title; ?>">
                Curso <?= $course_title; ?>
            </a>
            <span><span class="icon-arrow-right icon-notext"></span></span>
            <a href="dashboard.php?wc=cursos/curso&slug=<?= $course_name; ?>#<?= $module_name; ?>" title="<?= $module_title; ?>">
                Módulo <?= $module_title; ?>
            </a>
        </p>
    </div>
    ...
    <div class="wc_ead_content">
        <div class="wc_ead_course_task_media_play jwc_play_task">
            ...
            <div class="embed-container">
                <?php if ($class_video): ?>
                    <iframe src="<?= $class_video ?>" style="border:none;" allow="accelerometer;gyroscope;autoplay;encrypted-media;picture-in-picture" allowfullscreen=true width="720" height="360" fetchpriority="high"></iframe>
                <?php else:
                    echo "<div class='trigger trigger_info trigger_none al_center icon-info'>Ainda não existe vídeo para esta aula!</div>";
                endif; ?>
            </div>
        </div>
        ...
        if ($class_material):
            ...
            <a class='btn btn_cta_green icon-download' href='dashboard.php?wc=cursos/download&f={$class_id}' target='_blank' title='Baixar Material de Apoio'> CLIQUE AQUI PARA BAIXAR O MATERIAL</a> 
            </div>";
        endif;
        ?>
...
?>
    <article class="wc_ead_course_task_content">
        <div class="wc_ead_content">
            ...
            <aside class="box box_bar wc_ead_course_task_content_sidebar">
...
?>
    ...
    <section class="wc_ead_course_task_forum">
        <div class="wc_ead_content">
            ...
            <p class="al_right" style="padding-bottom: 40px;"><span class="btn btn_green btn_xxlarge icon-bubbles3 m_right wc_ead_allsupport">Todas os Comentários</span><span class="btn btn_blue btn_xxlarge icon-bubbles2 wc_ead_mysupport">Meus Comentários</span>
            <p>
            ...
            <div class="jwc_ead_forum_content jwc_allsupport">
                ...
                ?>
                        <article class="wc_ead_course_task_forum_ticket" id="<?= $Ticket['support_id']; ?>">
                            ...
                            <div class="wc_ead_course_task_forum_ticket_content">
        ...
        <article class="wc_ead_course_task_forum_ticket_new">
            <div class="wc_ead_content" style="padding-top: 0;">
                <header>
                    <h1 class="icon-bubbles2">Envie seu comentário:</h1>
                    <p>Envie seu comentário <b>sobre <?= $class_title; ?>!</b></p>
                </header>
                ...
                <form name="wc_ead_student_task_ticket_add" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form_actions">
                        <button class="btn btn_blue icon-bubble">ENVIAR!</button>
