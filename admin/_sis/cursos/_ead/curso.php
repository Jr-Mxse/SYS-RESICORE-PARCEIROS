    ...
    if (!$Modules):
        ...
        foreach ($Modules as $MOD):
            ...
            $Read->FullRead("(SELECT "
                    . "class_id "
                    . "FROM ead_classes class "
                    . "WHERE class.module_id IN (SELECT modu.module_id FROM ead_modules modu WHERE modu.course_id = {$course_id} AND modu.module_order < {$module_order} AND modu.module_required = 1) "
                    . "AND class.class_id NOT IN (SELECT stclass.class_id FROM ead_student_classes stclass WHERE stclass.student_class_check IS NOT NULL AND stclass.user_id = {$user_id} AND stclass.course_id = {$course_id} AND stclass.class_id IN (SELECT classmod.class_id FROM ead_classes classmod WHERE classmod.module_id IN (SELECT modreq.module_id FROM ead_modules modreq WHERE modreq.course_id = {$course_id} AND modreq.module_order < {$module_order} AND modreq.module_required = 1))))");
