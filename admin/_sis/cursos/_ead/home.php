...
if ($Read->getResult()):
    foreach ($Read->getResult() as $Course):
        $Read->ExeRead(DB_EAD_ENROLLMENTS, " WHERE course_id={$Course['course_id']} AND user_id={$_SESSION['userLogin']['user_id']}", "");
        if (!$Read->getResult()):
            $CreateUserCourse = ['user_id' => $_SESSION['userLogin']['user_id'], 'course_id' => $Course['course_id'], 'enrollment_start' => date('Y-m-d H:i:s')];
            $Create->ExeCreate(DB_EAD_ENROLLMENTS, $CreateUserCourse);
        endif;
    endforeach;
endif;
*/
    ...
    <article class="wc_ead_home_student">
        ...
        <div class="wc_ead_home_student_content">
            <div class="wc_ead_home_student_nav">
                <a href="#courses" class="wc_tab wc_active icon-books">Meus Cursos</a>
                <?php /*<a href="#orders" class="wc_tab icon-forward">ResiPlace</a>*/ ?>
            </div>
