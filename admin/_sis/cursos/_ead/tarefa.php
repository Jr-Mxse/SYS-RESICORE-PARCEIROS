...
if ($Read->getResult()):
    ...
    if (!$class_video):
        $CreateStudentClass['student_class_views'] = 1;
...
if ($Read->getResult()):
    ...
    $ClassNextBot = BASE . "/campus/tarefa/{$Read->getResult()[0]['class_name']}#play";
    ...
    if ($Read->getResult()):
        ...
        if ($CheckModuleUnlocked):
            ...
            $ClassNextBot = BASE . "/campus/curso/{$course_name}#{$ClassNextModule['module_name']}";
        endif;
    endif;
endif;
...
//Modelo Auto Play / Bot de Aula
if (isset($_GET["tp"])):
    if ($_GET["tp"] == "btn"):
        if ($ClassNextBot):
            $ClassNextBot = str_replace("#play", "", $ClassNextBot);
            $ClassNextBot .= "&tp=btn#play";
            $botTime = (($class_time + rand(1, 5)) * 60 * 1000) + rand(0, 10000);
            echo "<script>
(function() {
    var botTime   = {$botTime};
    var redirectUrl = '{$ClassNextBot}';
    var startTime = Date.now();
    var endTime   = startTime + botTime;

    console.log((botTime / 1000).toFixed(0) + ' segundos...');

    var countdown = setInterval(function() {
        var now = Date.now();
        var diff = Math.max(0, Math.floor((endTime - now) / 1000)); // segundos restantes

        if (diff > 0) {
            // mostra em formato mm:ss
            var minutes = Math.floor(diff / 60);
            var seconds = diff % 60;
            console.log('Faltam ' + minutes + 'm ' + (seconds < 10 ? '0' : '') + seconds + 's para redirecionar...');
        } else {
