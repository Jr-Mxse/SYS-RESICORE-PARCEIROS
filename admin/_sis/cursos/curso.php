...
if (!$CourseName):
    ...
    if (!$Read->getResult()):
        ...
        $TaskRedirect = filter_input(INPUT_GET, 'task', FILTER_SANITIZE_STRING);
        if ($TaskRedirect):
            header("Location: dashboard.php?wc=cursos/tarefa&task={$TaskRedirect}#play");
            exit;
        endif;
...
<div class="dashboard_content" style="padding-top: 10px;">
    <?php
    $Read->ExeRead(DB_EAD_MODULES, "WHERE course_id = :course ORDER BY module_order ASC", "course={$course_id}");
    $Modules = $Read->getResult();

    if (!$Modules):
        echo "<div class='trigger trigger_info trigger_none al_center icon-info m_top'>
                Ainda não existem módulos cadastrados para o curso {$course_title}!
              </div>";
    else:
        foreach ($Modules as $MOD):
            extract($MOD);

            $Read->FullRead("(SELECT class_id FROM ead_classes class
                WHERE class.module_id IN (
                    SELECT modu.module_id FROM ead_modules modu 
                    WHERE modu.course_id = {$course_id} 
                    AND modu.module_order < {$module_order} 
                    AND modu.module_required = 1
                )
                AND class.class_id NOT IN (
                    SELECT stclass.class_id FROM ead_student_classes stclass 
                    WHERE stclass.student_class_check IS NOT NULL 
                    AND stclass.user_id = {$user_id} 
                    AND stclass.course_id = {$course_id} 
                    AND stclass.class_id IN (
                        SELECT classmod.class_id FROM ead_classes classmod 
                        WHERE classmod.module_id IN (
                            SELECT modreq.module_id FROM ead_modules modreq 
                            WHERE modreq.course_id = {$course_id} 
                            AND modreq.module_order < {$module_order} 
                            AND modreq.module_required = 1
                        )
                    )
                ))");
            ...
            if ($module_required == 1):
                ...
                if ($ClassesChecked < $ClassesModule):
                    $barRequired = "<span class='bar_red radius icon-flag wc_tooltip'>
                                        Módulo obrigatório
                                        <span style='font-size: 1.2em;' class='wc_tooltip_balloon'>
                                            Para assistir as demais aulas, conclua todas deste módulo.
                                        </span>
                                    </span>";
                endif;
            endif;
            ...
            <section class="wc_ead_course_module" id="<?= $module_name; ?>">
                ...
                if (!$Read->getResult()):
                    echo "<div class='trigger trigger_info trigger_none al_center icon-info'>
                            Ainda não existem aulas cadastradas para o módulo {$module_title}!
                          </div>";
                else:
                        ...
                        <article class="wc_ead_course_module_class <?= (!empty($ClassPending) && $ClassPending['class_id'] == $class_id ? 'active' : ''); ?>">
                            <h1 class="row">
                                <span class="icon-play2">
                                    <?= (($ModuleUnlocked || $_SESSION['userLogin']['user_level'] > 5)
                                        ? "<a href='dashboard.php?wc=cursos/tarefa&task={$class_name}' 
