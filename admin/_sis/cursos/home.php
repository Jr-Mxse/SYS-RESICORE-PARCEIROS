						...
						if ($Read->getResult()):
							...
							foreach ($Read->getResult() as $Bonus):
								$Read->FullRead(
									...
									"user={$Admin['user_id']}&course={$Bonus['bonus_course_id']}"
								);
								...
								if ($Read->getResult()):
									if (!in_array($Bonus['course_title'], $EnrollmentUpdateKey)):
										...
										$Update->ExeUpdate(DB_EAD_ENROLLMENTS, $UpdateBonus, "WHERE user_id = :user AND enrollment_id = :enroll", "user={$Admin['user_id']}&enroll={$Read->getResult()[0]['enrollment_id']}");
									endif;
								else:
									...
									if (($BonusDiff->h <= 1 || $BonusDiff->invert) && empty($EnrollmentFree)):
										...
										$CreateUserBonus = ['user_id' => $Admin['user_id'], 'course_id' => $Bonus['bonus_course_id'], 'enrollment_bonus' => $Bonus['enrollment_id'], 'enrollment_start' => date('Y-m-d H:i:s'), 'enrollment_end' => $Bonus['enrollment_end']];
