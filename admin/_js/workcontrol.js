$(function () {
    ...
    // Máscara simples do CEP: 00000-000
    $(document).on('input', '.wc_cep', function () {
        let v = $(this).val().replace(/\D/g, '').slice(0, 8);
        if (v.length > 5) v = v.slice(0, 5) + '-' + v.slice(5);
        $(this).val(v);
    });

    $(document).on('blur input', '.wc_cep', function () {
        const raw = $(this).val().replace(/\D/g, '');
        if (raw.length !== 8) return;

        const $cep = $(this);
        $cep.prop('disabled', true).addClass('is_loading');
        ...
        $.getJSON(`https://viacep.com.br/ws/${raw}/json/`, function (data) {
            ...
            $cep.prop('disabled', false).removeClass('is_loading');
        });
    });
    ...
    //ARRASTE E SOLTE NO FILE
    if ($('.file_input_hidden').length) {

        $('.file_upload_area').on('dragover dragenter', function (e) {
            e.preventDefault();
            $(this).addClass('drag_over');
        });

        $('.file_upload_area').on('dragleave dragend', function (e) {
            e.preventDefault();
        ...
        $('.file_upload_area').on('drop', function (e) {
            ...
            if (files.length > 0) {
                const input = $(this).siblings('.file_input_hidden')[0];
                input.files = files;
...
function toPercentBR(str) {
    if (str == null) return 0;
    str = String(str).trim();
    if (!str) return 0;
    if (/%$/.test(str)) return toNumberBR(str.replace('%', '')) / 100;
    let n = toNumberBR(str);
    if (n > 1) n = n / 100;          // 20 -> 0.20
...
function safeDiv(num, den) {
    ...
    den = isFinite(den) && den !== 0 ? den : NaN;
    return isNaN(den) ? 0 : (num / den);
}