...
if ((!APP_USERS && !APP_EAD) || empty($_SESSION['userLoginParceiros']) || empty($_SESSION['userLoginParceiros']['user_level']) || $_SESSION['userLoginParceiros']['user_level'] < $NivelAcess):
...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack):
    ...
    switch ($Case):
        ...
        case 'manager':
            ...
            if ($Read->getResult()):
                $jSON['trigger'] = AjaxErro("<b class='icon-warning'>OPSS:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}. O e-mail <b>{$PostData['user_email']}</b> já está cadastrado na conta de outro usuário!", E_USER_WARNING);
            else:
                $Read->FullRead("SELECT user_id FROM " . DB_USERS . " WHERE user_document = :dc AND user_id != :id", "dc={$PostData['user_document']}&id={$UserId}");
                //if ($Read->getResult()):
                //$jSON['trigger'] = AjaxErro("<b class='icon-warning'>OPSS:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}. O CPF <b>{$PostData['user_document']}</b> já está cadastrado na conta de outro usuário!", E_USER_WARNING);
                //else:
                /*
                    if (Check::CPF($PostData['user_document']) != true):
                        $jSON['trigger'] = AjaxErro("<b class='icon-warning'>OPSS:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}. O CPF <b>{$PostData['user_document']}</b> informado não é válido!", E_USER_WARNING);
                        echo json_encode($jSON);
                        return;
                    endif;*/
                ...
                if (!empty($_FILES['user_thumb'])):
                    ...
                    if ($Upload->getResult()):
                        ...
                        $jSON['trigger'] = AjaxErro("<b class='icon-image'>ERRO AO ENVIAR FOTO:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}, selecione uma imagem JPG ou PNG para enviar como foto!", E_USER_WARNING);
                        echo json_encode($jSON);
                        return;
                    endif;
                endif;
                ...
                if (!empty($PostData['user_password'])):
                    if (strlen($PostData['user_password']) >= 5):
                        ...
                        $jSON['trigger'] = AjaxErro("<b class='icon-warning'>ERRO DE SENHA:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}, a senha deve ter no mínimo 5 caracteres para ser redefinida!", E_USER_WARNING);
                        echo json_encode($jSON);
                        return;
                    endif;
                else:
                ...
                if ($UserId == $_SESSION['userLoginParceiros']['user_id']):
                    if ($PostData['user_level'] != $_SESSION['userLoginParceiros']['user_level']):
                        $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>PERFIL ATUALIZADO COM SUCESSO:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}, seus dados foram atualizados com sucesso!<p class='icon-warning'>Seu nível de usuário não foi alterado pois não é permitido atualizar o próprio nível de acesso!</p>");
                    else:
                        $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>PERFIL ATUALIZADO COM SUCESSO:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}, seus dados foram atualizados com sucesso!");
                    endif;
                    $SesseionRenew = true;
                    unset($PostData['user_level']);
                elseif ($PostData['user_level'] > $_SESSION['userLoginParceiros']['user_level']):
                    $PostData['user_level'] = $_SESSION['userLoginParceiros']['user_level'];
                    $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>TUDO CERTO:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}. O usuário {$PostData['user_name']} {$PostData['user_lastname']} foi atualizado com sucesso!<p class='icon-warning'>Você não pode criar usuários com nível de acesso maior que o seu. Então o nível gravado foi " . getWcLevel($PostData['user_level']) . "!</p>");
                else:
                    $jSON['trigger'] = AjaxErro("<b class='icon-checkmark'>TUDO CERTO:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}. O usuário {$PostData['user_name']} {$PostData['user_lastname']} foi atualizado com sucesso!");
                endif;
        ...
        case 'delete':
            ...
            if (!$Read->getResult()):
                $jSON['trigger'] = AjaxErro("<b class='icon-warning'>USUÁRIO NÃO EXISTE:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}, você tentou deletar um usuário que não existe ou já foi removido!", E_USER_WARNING);
            else:
                extract($Read->getResult()[0]);
                if ($user_id == $_SESSION['userLoginParceiros']['user_id']):
                    $jSON['trigger'] = AjaxErro("<b class='icon-warning'>OPPSSS:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}, por questões de segurança, o sistema não permite que você remova sua própria conta!", E_USER_WARNING);
                elseif ($user_level > $_SESSION['userLoginParceiros']['user_level']):
                    $jSON['trigger'] = AjaxErro("<b class='icon-warning'>PERMISSÃO NEGADA:</b> Desculpe {$_SESSION['userLoginParceiros']['user_name']}, mas {$user_name} tem acesso superior ao seu. Você não pode remove-lo!", E_USER_WARNING);
                else:
        ...
        case 'addr_delete':
            ...
            if ($Read->getResult()):
                $jSON['trigger'] = AjaxErro("<b class='icon-warning'>ERRO AO DELETAR:</b> Olá {$_SESSION['userLoginParceiros']['user_name']}, deletar um registro vinculado a pedidos não é permitido pelo sistema!", E_USER_WARNING);
            else:
        ...
        case 'block_user':
            ...
            $MailBody = "
                    <p style=''>Olá {$Student['user_name']},</p>
            ...
            $jSON['redirect'] = 'dashboard.php?wc=users/create&id=' . $PostData['user_id'];
            $jSON['success'] = true;
            $jSON['clear'] = true;
            break;
