...
if ($POST && $POST['callback']):
    ...
    switch ($Callback):
        ...
        case 'wc_ead_student_task_manager_check':
            ...
            if (!$user_id || !$studend_class_id):
                ...
                if ($Read->getResult()):
                    ...
                    $jSON['check'] = "<span class='a active icon-checkmark jwc_ead_task_uncheck'>" . date("d/m/Y H\hi") . "</span>";
                    $jSON['trigger'] = AjaxErro("Tarefa concluída {$_SESSION['userLogin']['user_name']} Parabéns você concluiu a tarefa {$Read->getResult()[0]['class_title']}!", E_USER_NOTICE);
                else:
        ...
        case 'wc_ead_student_task_manager_uncheck':
            ...
            if (!$user_id || !$studend_class_id):
                $jSON['trigger'] = AjaxErro("Erro ao concluir tarefa: <p>Desculpe, não foi possível identificar seu login ou a tarefa acessada. As aulas devem ser feitas uma de cada vez.</p>", E_USER_WARNING);            else:
                $Read->FullRead("SELECT class_title FROM " . DB_EAD_CLASSES . " WHERE class_id = (SELECT class_id FROM " . DB_EAD_STUDENT_CLASSES . " WHERE student_class_id = :class)", "class={$studend_class_id}");
                if ($Read->getResult()):
                    ...
                    $jSON['trigger'] = AjaxErro("Volte aqui depois {$_SESSION['userLogin']['user_name']}, {$Read->getResult()[0]['class_title']}!", E_USER_NOTICE);
                else:
                    $jSON['trigger'] = AjaxErro("Tarefa não identificada {$_SESSION['userLogin']['user_name']}, Atualize a página para que a tarefa seja identificada!", E_USER_WARNING);
                endif;
            endif;
        ...
        case 'wc_ead_student_task_ticket_add':
            ...
            if (empty($TicketContent)):
                ...
                if (!$Read->getResult()):
                    ...
                    if (!$Read->getResult()):
                        $CreateTicket = ['user_id' => $_SESSION['userLogin']['user_id'], 'enrollment_id' => $_SESSION['wc_student_enrollment_id'], 'class_id' => $SupportClass['class_id'], 'support_content' => $TicketContent, 'support_status' => 3, 'support_open' => date("Y-m-d H:i:s")];
                        ...
                        $jSON['ead_support_content'] = "<article class='wc_ead_course_task_forum_ticket' id='{$Create->getResult()}'>
                            <div class='wc_ead_course_task_forum_ticket_thumb " . ($_SESSION['userLogin']['user_level'] > 5 ? 'admin' : '') . "'>
                                ...
                                <header class='wc_ead_course_task_forum_ticket_header'>
                                    <h1 class='icon-bubble2'><span class='user'>Pergunta de {$_SESSION['userLogin']['user_name']} {$_SESSION['userLogin']['user_lastname']}</span> <span class='time'>dia " . date('d/m/Y H\hi') . "</span></h1>
                                </header>
