...
if ($PostData && $PostData['callback_action'] && $PostData['callback'] == $CallBack):
    ...
    if (file_exists("../dashboard.json")):
        ...
        if (!empty($LicenseDomain->license_auth_date)):
            ...
            if ($DateDiff >= 5):
                ...
                try {
                    $PostLicence = file_get_contents("https://download.workcontrol.com.br?h={$LicenseDomain->license_hash}&d=" . urlencode(BASE2));
    ...
    switch ($Case):
            //WC LOGIN FIX
        case 'wc_login_fix':
            if (!empty($_SESSION['userLoginParceiros']) && $_SESSION['userLoginParceiros']['user_level'] >= 6):
                $Read->ExeRead(DB_USERS, "WHERE user_id = :user", "user={$_SESSION['userLoginParceiros']['user_id']}");
        ...
        case 'onlinenow':
            ...
            if (!$Read->getResult()):
                ...
                foreach ($Read->getResult() as $Online):
                    $i++;
                    $Name = ($Online['online_name'] ? "<a href='dashboard.php?wc=users/create&id={$Online['online_user']}' title='Ver Lead'>{$Online['online_name']}</a>" : 'guest user');
                    $Date = date('d/m/Y H\hi', strtotime($Online['online_startview']));
                    $jSON['data'] .= "<div class='single_onlinenow'>
                    <p>" . str_pad($i, 4, 0, STR_PAD_LEFT) . "</p>
                    <p><a href='" . BASE2 . "/admin/dashboard.php?wc=onlinenow&user={$Online['online_user']}' class='btn btn_green btn_small icon-notext icon-filter'></a> {$Name}</p>
                    <p>{$Date}</p>
                    <p>{$Online['online_ip']}</p>
                    <p><a href='" . BASE2 . "/admin/dashboard.php?wc=onlinenow&url={$Online['online_url']}' class='btn btn_green btn_small icon-notext icon-filter'></a> <a target='_blank' href='" . BASE2 . "/{$Online['online_url']}' title='Ver Destino'>" . ($Online['online_url'] ? $Online['online_url'] : 'home') . "</a></p>
                    </div>";
                endforeach;
            endif;
            break;
