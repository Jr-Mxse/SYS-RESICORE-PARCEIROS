...
class Session {
    ...
    private function sessionStart() {
        ...
        if (!empty($_SESSION['userLoginParceiros'])):
            $this->Session['online_user'] = (!empty($_SESSION['userLoginParceiros']['user_id']) ? $_SESSION['userLoginParceiros']['user_id'] : null);
            $this->Session['online_name'] = (!empty($_SESSION['userLoginParceiros']['user_name']) && !empty($_SESSION['userLoginParceiros']['user_lastname']) ? "{$_SESSION['userLoginParceiros']['user_name']} {$_SESSION['userLoginParceiros']['user_lastname']}" : null);
        endif;
    ...
    private function viewsStart() {
        ...
        if ($Read->getResult()):
            ...
            //24 HORS TO NEW USER
            setcookie('userView', Check::Name(SITE_NAME2), time() + 86400, '/');
        else:
