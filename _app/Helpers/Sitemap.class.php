...
class Sitemap {
    ...
    private function SitemapUpdate() {
        ...
        if (APP_PAGES):
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $ReadPages):
                    $this->Sitemap .= '<url>' . "\r\n";
                    $this->Sitemap .= '<loc>' . BASE2 . '/' . $ReadPages['page_name'] . '</loc>' . "\r\n";
        ...
        if (APP_POSTS):
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $ReadPages):
                    $this->Sitemap .= '<url>' . "\r\n";
                    $this->Sitemap .= '<loc>' . BASE2 . '/artigos/' . $ReadPages['category_name'] . '</loc>' . "\r\n";
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $ReadPages):
                    $this->Sitemap .= '<url>' . "\r\n";
                    $this->Sitemap .= '<loc>' . BASE2 . '/artigo/' . $ReadPages['post_name'] . '</loc>' . "\r\n";
        ...
        if (APP_PRODUCTS):
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $ReadPages):
                    $this->Sitemap .= '<url>' . "\r\n";
                    $this->Sitemap .= '<loc>' . BASE2 . '/produtos/' . $ReadPages['cat_name'] . '</loc>' . "\r\n";
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $ReadPages):
                    $this->Sitemap .= '<url>' . "\r\n";
                    $this->Sitemap .= '<loc>' . BASE2 . '/produto/' . $ReadPages['pdt_name'] . '</loc>' . "\r\n";
            ...
            if ($Read->getResult()):
                foreach ($Read->getResult() as $ReadPages):
                    $this->Sitemap .= '<url>' . "\r\n";
                    $this->Sitemap .= '<loc>' . BASE2 . '/marca/' . $ReadPages['brand_name'] . '</loc>' . "\r\n";
    ...
    private function SitemapPing() {
        ...
        $this->SitemapPing['g'] = 'https://www.google.com/webmasters/tools/ping?sitemap=' . urlencode(BASE2 . '/sitemap.xml');
        $this->SitemapPing['b'] = 'https://www.bing.com/webmaster/ping.aspx?siteMap=' . urlencode(BASE2 . '/sitemap.xml');
