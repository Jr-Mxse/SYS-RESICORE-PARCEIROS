...
echo '<title>' . SITE_NAME2 . ' - ' . SITE_SUBNAME . '</title>' . "\r\n";
echo '<link>' . BASE2 . '</link>' . "\r\n";
    ...
    case('products'):
        ...
        if ($Read->getResult()):
            foreach ($Read->getResult() as $ReadPdt):
                ...
                echo '<link>' . BASE2 . '/produto/' . $ReadPdt['pdt_name'] . '</link>' . "\r\n";
                ...
                echo '<enclosure type="image/*" url="' . BASE2 . '/uploads/' . $ReadPdt['pdt_cover'] . '"/>' . "\r\n";

                //FB ADS PRODUCTS
                echo '<id>product_' . $ReadPdt['pdt_id'] . '</id>' . "\r\n";
                echo '<image_link>' . BASE2 . "/uploads/" . $ReadPdt['pdt_cover'] . '</image_link>' . "\r\n";
                ...
                $PdtGetBrand = ($Read->getResult() ? $Read->getResult()[0]['brand_title'] : SITE_NAME2);
    ...
    case('realty'):
        ...
        if ($Read->getResult()):
            foreach ($Read->getResult() as $ReadRealty):
                ...
                echo '<link>' . BASE2 . '/imovel/' . $ReadRealty['realty_name'] . '</link>' . "\r\n";
                ...
                echo '<enclosure type="image/*" url="' . BASE2 . '/uploads/' . $ReadRealty['realty_cover'] . '"/>' . "\r\n";

                //FB ADS PRODUCTS
                echo '<id>realty_' . $ReadRealty['realty_id'] . '</id>' . "\r\n";
                echo '<image_link>' . BASE2 . "/uploads/" . $ReadRealty['realty_cover'] . '</image_link>' . "\r\n";
                echo '<condition>new</condition>' . "\r\n";
                echo '<price>' . ($ReadRealty['realty_price'] ? $ReadRealty['realty_price'] : '0.00') . '</price>' . "\r\n";
                echo '<availability>in stock</availability>' . "\r\n";
                echo '<brand>' . SITE_NAME2 . '</brand>' . "\r\n";
    ...
    case('courses'):
        ...
        if ($Read->getResult()):
            foreach ($Read->getResult() as $ReadCourse):
                ...
                echo '<link>' . BASE2 . "/curso/{$ReadCourse['course_name']}" . '</link>' . "\r\n";
                ...
                echo '<enclosure type="image/*" url="' . BASE2 . '/uploads/' . $ReadCourse['course_cover'] . '"/>' . "\r\n";

                //FB ADS PRODUCTS
                echo '<id>course_' . $ReadCourse['course_id'] . '</id>' . "\r\n";
                echo '<image_link>' . BASE2 . "/uploads/" . $ReadCourse['course_cover'] . '</image_link>' . "\r\n";
                echo '<condition>new</condition>' . "\r\n";
            ...
            foreach ($Read->getResult() as $ReadPosts):
                ...
                if ($Read->getResult()) {
                    foreach ($Read->getResult() as $slide):
                        $Slides[] = BASE2 . '/uploads/' . $slide['image'];
                    endforeach;
                }
                ...
                if ($Read->getResult()) {
                    foreach ($Read->getResult() as $rel):
                        $ArtRel[] = BASE2 . '/artigo/' . $rel['post_name'];
                    endforeach;
                }
                ...
                $Content = [
                    'canonical' => BASE2 . '/artigo/' . $ReadPosts['post_name'], // Obrigatório
                    'title' => $ReadPosts['post_title'], // Obrigatório
                    'desc' => str_replace('&', 'e', $ReadPosts['post_subtitle']), // Obrigatório
                    'capa' => BASE2 . '/uploads/' . $ReadPosts['post_cover'], // Obrigatório
                    ...
                    'SITE_NAME2' => SITE_NAME2, // Obrigatório
                ...
                echo '<link>' . BASE2 . '/artigo/' . $ReadPosts['post_name'] . '</link>' . "\r\n";
                ...
                echo '<enclosure type="image/*" url="' . BASE2 . '/uploads/' . $ReadPosts['post_cover'] . '"/>' . "\r\n";
